//
//  WYComponentTableViewController.m
...
#import "WYComponentTableViewController.h"
...
@interface WYComponentTableItem(Privated)
...
//------------------------------------------------------------
@protocol WYComponentTableViewDelegate <NSObject>
...
//------------------------------------------------------------
@interface WYComponentTableView : UITableView

@property (nonatomic, weak) id<WYComponentTableViewDelegate> privateDelegate;
@property (nonatomic, assign) BOOL disableSetContentOffset;

@end

@implementation WYComponentTableView
...
//------------------------------------------------------------
@interface WWKIgnoreMemoryWarningTableView : WYComponentTableView
...
//------------------------------------------------------------
@interface WYComponentDisappearHover : NSObject

@property (nonatomic, strong) WYComponentTableItem *hoverItem;
@property (nonatomic, strong) UITableViewCell *hoverView;

@end

@implementation WYComponentDisappearHover
@end

//------------------------------------------------------------
@interface WYComponentTableKeyboardInfo : NSObject
...
@implementation WYComponentTableKeyboardInfo

@end

//------------------------------------------------------------
@interface WYComponentTableViewController () <UIGestureRecognizerDelegate, WYComponentTableViewDelegate>

@property (nonatomic, strong) NSMutableDictionary *topHoverItemMap;
@property (nonatomic, strong) NSMutableArray<WYComponentDisappearHover *> *disappearHoverCellArr;
@property (nonatomic, strong) WYComponentTableItem *currentTopHoverItem;
@property (nonatomic, strong) UITableViewCell *currentTopHoverView;
@property (nonatomic, assign) EasyHoverState topHoverState;

@property (nonatomic, strong) WYComponentTableKeyboardInfo *keyboardInfo;

@property (nonatomic, strong) UITapGestureRecognizer *tap;

@property (nonatomic, weak) WYComponentTableItem *fixedSpaceItem;   //用于撑满屏幕高度时候用，平时用的不多
@property (nonatomic, assign) BOOL fixedSpaceReloading;

@end

@implementation WYComponentTableViewController
{
...
- (void)dealloc {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"WYComponentTableItemLayout" object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"WYComponentTableItemFixedSpace" object:nil];
}

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.inputHandleOption = WYComponentKeyboardHandleOptionNone;
    [self _setupUI];
    [self _addKeyboardNoti];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleEditCellFrameWhenEditing) name:@"WYComponentTableItemLayout" object:self.tableView];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleTableItemFixedSpace:) name:@"WYComponentTableItemFixedSpace" object:nil];
}
...
- (void)setInputHandleOption:(WYComponentInputHandleOption)inputHandleOption {
...
- (BOOL)shouldHandleKeyboard {
    if (self.inputHandleOption==WYComponentKeyboardHandleOptionNone ||
        self.inputHandleOption==WYComponentKeyboardHandleOptionSystem) {
        return NO;
    }
...
- (void)_setupUI {
    ...
    self.tableView = [[WYComponentTableView alloc] initWithFrame:CGRectZero style:[self tableViewStyle]];
    ((WYComponentTableView*)self.tableView).disableSetContentOffset = self.disableTableViewSetContentOffset;
    [(WYComponentTableView *)self.tableView setPrivateDelegate:self];
...
- (void)beforeReloadData {
    [self.dataSource enumerateObjectsUsingBlock:^(WYComponentTableSectionItem * _Nonnull sectionItem, NSUInteger idx, BOOL * _Nonnull stop) {
        [sectionItem.cellDataSource enumerateObjectsUsingBlock:^(WYComponentTableItem * _Nonnull obj, NSUInteger idx2, BOOL * _Nonnull stop2) {
...
- (void)afterReloadData {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            for (WYComponentTableSectionItem *obj in self.dataSource) {
                ...
                for (WYComponentTableItem *item in obj.cellDataSource) {
...
- (void)_addKeyboardNoti {
    if (WYComponentKeyboardHandleOptionDisable ==_inputHandleOption) {
        return;
    }
...
- (NSMutableArray<WYComponentTableSectionItem *> *)dataSource {
    if (!_dataSource) {
        _dataSource = (NSMutableArray<WYComponentTableSectionItem *> *)[NSMutableArray array];
    }
    return _dataSource;
}

- (NSMutableDictionary<NSString *, WYComponentTableItem *> *)topHoverItemMap {
    if (!_topHoverItemMap) {
        _topHoverItemMap = (NSMutableDictionary<NSString *, WYComponentTableItem *> *)[NSMutableDictionary dictionary];
    }
...
- (void)handleEditCellFrameWhenEditing {
    ...
    if (self.inputHandleOption == WYComponentKeyboardHandleOptionCursor) { // 查找光标的位置
...
- (void)easykeyboardWillShow:(NSNotification *)noti {
    ...
    // 构造键盘信息
    WYComponentTableKeyboardInfo *keyboardInfo = [WYComponentTableKeyboardInfo new];
    ...
    if (self.inputHandleOption==WYComponentKeyboardHandleOptionSystem) {
    ...
    if (self.inputHandleOption == WYComponentKeyboardHandleOptionCursor) { // 通知时候的光标位置是老位置，在此时无法获取最新的光标位置，所以延时处理
        self.keyboardInfo.didChangeContentInset = YES;
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self handleEditCellFrameWhenEditing];
        });
    } else if (self.inputHandleOption == WYComponentKeyboardHandleOptionCell) {
        self.keyboardInfo.didChangeContentInset = YES;
        [self handleEditCellFrameWhenEditing];
    }
}
...
-(void)setIgnoreMemoryWarning:(BOOL)ignoreMemoryWarning{
    ...
    if (ignoreMemoryWarning) {
        ...
        object_setClass(self.tableView, [WYComponentTableView class]);
    }
}
...
- (void)setDisableTableViewSetContentOffset:(BOOL)disableTableViewSetContentOffset {
    ...
    if ( self.isViewLoaded && _tableView != nil && [_tableView isKindOfClass:[WYComponentTableView class]] ) {
        ((WYComponentTableView*)_tableView).disableSetContentOffset = disableTableViewSetContentOffset;
    }
}
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if (gestureRecognizer == self.tap) {
        if (self->_didShowKeybord && self.endEditingWhenTouch && self.inputHandleOption != WYComponentKeyboardHandleOptionDisable) {
...
- (UITableViewCell *)cellForItem:(WYComponentTableItem *)item {
    ...
    if (!cell) {
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"WYComponentTableViewDefaultCell"];
    }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    WYComponentTableSectionItem *sectionItem = [self.dataSource objectAtIndex:section];
    return sectionItem.cellDataSource.count;
}

- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    WYComponentTableSectionItem<NSArray *> *sectionItem = [self.dataSource objectAtIndex:section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    WYComponentTableSectionItem *sectionItem = [self.dataSource objectAtIndex:indexPath.section];
    WYComponentTableItem *item = [sectionItem.cellDataSource objectAtIndex:indexPath.row];
...
- (WYComponentTableItem *)minTopHoverItem {
    NSInteger maxRow = 0;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    // 1.找到最近那个cell
    WYComponentTableItem *replaceItem = [self minTopHoverItem];
    ...
    if (self.currentTopHoverItem != replaceItem) { // 探测到新的一个item
            ...
            if (!self.currentTopHoverView) {
                ...
                if (replaceCellRectTop < scrollView.contentOffset.y) {
                    if ([self setTopHoverStateIsRefresh:HoverStateSuspend]) {
                        // 老的刚好被滚走了
                        [self.currentTopHoverView removeFromSuperview];
                        WYComponentDisappearHover *disappearHover = [WYComponentDisappearHover new];
        ...
        if (replaceCellRectBottom > hoverCellRectBottom) {
            if ([self setTopHoverStateIsRefresh:HoverStateReplace]) {
                // 刚好滚回来
                WYComponentDisappearHover *disappearHover = self.disappearHoverCellArr.lastObject;
                [self.currentTopHoverView removeFromSuperview];
                self.currentTopHoverView = nil;
...
-(void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    if (self.endEditingWhenBeginDragging && self.inputHandleOption != WYComponentKeyboardHandleOptionDisable) {
        [self.view endEditing:YES];
    }
}

// 根据WYComponentTableItem的filterID查询对应的WYComponentTableItem
- (NSArray<__kindof WYComponentTableItem *> *)getEasyTableItemsWithfilterID:(NSString *)filterID {
    // 这里没有用map效率比较低，但是map维护成本高，先这样用一下吧
    NSMutableArray<WYComponentTableItem *> *allItems = [NSMutableArray array];
    NSMutableArray<WYComponentTableItem *> *re = [NSMutableArray array];
    [self.dataSource enumerateObjectsUsingBlock:^(WYComponentTableSectionItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [allItems addObjectsFromArray:obj.cellDataSource];
    }];
    [allItems enumerateObjectsUsingBlock:^(WYComponentTableItem *obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
// 获取所有WYComponentTableItem
- (NSArray<__kindof WYComponentTableItem *> *)p_getAllEasyTableItems {
    // 这里没有用map效率比较低，但是map维护成本高，先这样用一下吧
    NSMutableArray<WYComponentTableItem *> *allItems = [NSMutableArray array];
    [self.dataSource enumerateObjectsUsingBlock:^(WYComponentTableSectionItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [allItems addObjectsFromArray:obj.cellDataSource];
