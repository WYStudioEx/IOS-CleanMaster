...
//  gifMerchantModule
//
//  Created by WYStudio on 2020/11/16.
...
- (void)configUIData {
    ...
    __weak typeof(self) weakSelf = self;
    int i = 0;
    WYComponentTableSectionItem *section = [WYComponentTableSectionItem new];
    for (KSTemplateListItem *it in self.viewModel.datas) {
        WYComponentTableItem *item = [[WYComponentTableItem alloc] init];
        ...
        i++;
        item.cellClickAction = ^(__kindof WYComponentTableItem * _Nonnull cellItem) {
            __strong typeof(&*weakSelf) strongSelf = weakSelf;
            if (nil == strongSelf) {
                return;
            }
        ...
        item.configCellBlock = ^(__kindof KSTemplateTableViewCell * _Nonnull cell, __kindof WYComponentTableItem * _Nonnull item) {
            __strong typeof(&*weakSelf) strongSelf = weakSelf;
            if (nil == strongSelf) {
                return;
            }
    ...
    section = [WYComponentTableSectionItem new];
